...
class Scheduling{               //스케줄링 함수
  ...
  public static int Timeslice;  //타임 슬라이스(사용자의 입력을 통해 정함)
...
//---------------------------------------------------------------------------
  public void inputTimeslice(){            //Timeslice를 입력받는 함수
    Scanner sc=new Scanner(System.in);     //Scanner를 통해 사용자로부터 입력을 받음
    System.out.printf("Enter Timeslice: ");//Timeslice를 입력하라는 문구를 띄움
...
//---------------------------------------------------------------------------
  ...
  public void startScheduling(){                        //스케줄링 시작
    ...
    while(queue.rear!=queue.front){                             //Queue가 비어있지 않은동안
      if(queue.circular_Queue[(queue.front+1)%4].rt<=Timeslice){//Queue[front+1]의 남은 시간이 Timeslice보다 작거나 같으면
        if(queue.circular_Queue[(queue.front+1)%4].at<=Time){   //Queue[front+1]의 도착시간이 현재 시간보다 작거나 같으면
          ...
          Time+=queue.circular_Queue[(queue.front+1)%4].rt; //전체 시간은 지난 전체 시간에 Queue[front+1]의 남은 시간을 더해준다(whole time=last whole time+rest time)
      ...
      //==============================================================================================================
      else{ //Queue[front+1]의 남은 시간이 Timeslice보다 크면(rt>Timeslice)
          if(queue.circular_Queue[(queue.front+1)%4].at<=Time){     //Queue[front+1]의 도착시간이 현재 시간보다 작거나 같으면
            queue.circular_Queue[(queue.front+1)%4].it += Timeslice;//현재 실행 시간에 Timeslice를 더한다
            System.out.printf("process%d!\n",queue.circular_Queue[(queue.front+1)%4].processname);//현재 진행하고 있는 프로세스의 name을 출력

            if((queue.front+1)%4!=queue.rear){  //만약 Queue[front+1]이 rear의 위치가 같지 않으면(Queue에 data가 존재한다면)(rest someting in queue)
              for(int k=2;k<=3;k++){
                if((queue.rear+1)%4 != (queue.front+k)%4){              //만약 Queue[rear+1]과 Queue[front+2] 그리고 Queue[front+3]의 위치가 같지 않으면
                  queue.circular_Queue[(queue.front+k)%4].wt+=Timeslice;//Queue[front+2] 그리고 Queue[front+3]의 대기 시간에 Timeslice를 더해준다
                }
              }
            }
            Time+=Timeslice;//전체 시간은 지난 전체 시간에 Timeslice를 더해준다
            queue.circular_Queue[(queue.front+1)%4].rt = queue.circular_Queue[(queue.front+1)%4].pt - queue.circular_Queue[(queue.front+1)%4].it;
            //Queue[front+1]의 남은 시간은 Queue[front+1]의 전체 시간에서 진행 시간만큼 뺀시간이므로 이를 계산한다(rt=pt-it)
            //
            System.out.printf("1: %d %d %d\n",p1.wt,p1.it,p1.rt);//실행되는 프로세스1의 wt, it, rt순으로 출력하여 스케줄링되는 과정을 확인할 수 있다
